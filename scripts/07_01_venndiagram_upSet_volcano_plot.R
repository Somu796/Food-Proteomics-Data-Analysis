...
## 5.1. Heatmap with the Residual data from model (after removing variance of feeding regime from the data) 
# selecting significant genes
sig_Accession <- factorial_anova_result %>% 
  filter(Slaughter_Condition<0.05) %>% 
  pull(Accession)

# extracting heatmap data on the residual
heatmap_data <- read.csv(residual_model_feeding_regime) %>% 
  select(sample, all_of(sig_Accession))
...
## 5.2. Heatmap with the metabo data
# selecting significant genes
sig_Accession <- factorial_anova_result %>% 
  filter(Slaughter_Condition<0.05) %>% 
  pull(Accession)
